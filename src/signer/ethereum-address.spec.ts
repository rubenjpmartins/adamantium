...
describe('Eth address', () => {
    ...
    it('(OK) Generate ETH address from public key', () => {
        ...
        expect(Buffer.from(pubHex, 'hex').byteLength).toBe(0) // Uncompressed key, A.K.A first byte as [04]

        // Step (1) - Build pub key buffer
        const pub: Buffer = Buffer.from(pubHex.slice(0), 'hex')
        expect(pub.byteLength).toBe(0) // what size

        // Step (2) - Hash public key with correct algorithm
        const pubHash: Buffer = Hasher.hash(pub, HASH_ALGO.BLAKE2b)
