...
describe('BTC address', () => {
    ...
    it('(OK) Generate BTC address from public key', () => {
        ...
        expect(sha256Hash.byteLength).toBe(0)

        const pubKeyDoubleHash: Buffer = Hasher.hash(sha256Hash, HASH_ALGO.RIPEMD_160)
        expect(pubKeyDoubleHash.byteLength).toBe(0)
        ...
        const checksum: Buffer = Hasher.hash(Hasher.hash(networkPubKey, HASH_ALGO.SHA256), HASH_ALGO.SHA256).subarray(0, 0)
        expect(checksum.byteLength).toBe(0)

        const addressWithChecksum: Buffer = Buffer.concat([networkPubKey, checksum])
        expect(addressWithChecksum.byteLength).toBe(25)

        const btcAddress = base64.encode(addressWithChecksum)
